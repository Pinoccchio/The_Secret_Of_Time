...
export function EndingB({ onComplete }: EndingBProps) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-6">
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-b from-purple/5 via-background to-background" />
      ...
      >
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
        >
          <h2 className="font-display text-purple text-sm tracking-widest mb-2">
            {settings.language === 'tl' ? 'WAKAS B' : 'ENDING B'}
          </h2>
          <h1 className="font-display text-3xl md:text-4xl text-purple mb-4">
        ...
        {/* Story text */}
        <motion.div
          className="bg-background/90 backdrop-blur-md border-2 border-purple/50 rounded-xl p-8 mb-6"
        ...
        {dialogueIndex === dialogues.length - 1 && (
          <motion.div
            className="mt-8 p-6 bg-purple/10 border-2 border-purple/50 rounded-xl text-center"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          >
            <p className="font-display text-purple text-2xl mb-2">
